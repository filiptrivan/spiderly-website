...
`[Required]` decides it — the same attribute EF Core turns into a NOT NULL column:
...
| `int Stock`               | `int? Stock`                                                                                                              |
| `[Required]` navigation   | `long CategoryId` — the FK follows the navigation. `CategoryDisplayName` stays nullable: it projects out of *another* entity, so deriving it would make a `[Required]` over there silently change this DTO's contract. |
| any collection            | unchanged, with `= new()`                                                                                                 |

<Callout>
  A required **value type** is non-nullable, so an explicit JSON `null` for it fails at model binding
  with a 400 rather than your usual validation 422 — `null` cannot be assigned to `int`. A *missing*
  property is fine (it lands as `default`). Required **reference types** have no such problem: `null`
  assigns, and the generated `.NotEmpty()` rule returns the normal 422 with field errors.
</Callout>
...
It is off by default because turning it on narrows your published contract — members become `required` and non-nullable, which regenerates every consumer's typed client. Treat it as its own deploy, coordinated with those consumers. Apps created by `spiderly init` have it on already, since a brand-new app has no consumers to coordinate with. The flag is also a no-op until your app is on `<Nullable>enable</Nullable>`.
