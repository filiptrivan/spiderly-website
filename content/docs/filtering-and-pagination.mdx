...
- A filter key that matches no generated case — silently ignoring it would return _unfiltered_ rows the caller treats as filtered.
- A `MultiSortMeta.Field` that matches no sort case.
- A `MatchMode` that isn't valid for the field's type.

The error message names the offending field and lists the valid ones, so both humans and AI agents can self-correct from the response alone.
