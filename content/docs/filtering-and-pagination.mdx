...
The generated query is always totally ordered: when `MultiSortMeta` is empty the backend falls back to `Id` descending (newest first), and when sort rules are present `Id` descending is appended as the final tie-breaker. Without this, `Skip`/`Take` on an unordered (or non-uniquely ordered) query returns rows in arbitrary database order, so pages could repeat or drop rows. M2M junction entities have no `Id` and are excluded. To show a different default sort in the UI, set `defaultSortField` on the data table (see [Data Components](/docs/angular-components/data-components)).

## Invalid filter and sort input

Client input the generated pipeline doesn't recognize is **rejected with HTTP 400** (`BusinessException`), never silently ignored:

| Input                                                                                                                       | `ApiErrorDTO.errorCode` |
| --------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| A filter key matching no generated case — silently ignoring it would return _unfiltered_ rows the caller treats as filtered | `unknown_filter_field`  |
| A `MultiSortMeta.Field` matching no sort case                                                                               | `unknown_sort_field`    |
| A `MatchMode` that isn't valid for the field's type                                                                         | `invalid_match_mode`    |

The message names the offending field and lists the valid ones, so humans and AI agents can self-correct from the response alone; branch on `errorCode` rather than the message text. The throws come from `PaginationErrors` (`Spiderly.Shared.Exceptions`), which is also where the wording lives.

Two consequences to design around:

- `*CommaSeparated` collection fields are **filterable but not sortable**. The data table auto-disables click-to-sort on such columns, so this only matters for hand-built API calls.
- A hand-written `GetPaginated{Entity}List` override that consumes a custom pseudo filter key must remove it before delegating — otherwise the generated base rejects the key:

```csharp
if (filterDTO.Filters.TryGetValue("customSkuSearch", out List<FilterRuleDTO> skuFilters))
{
    filterDTO.Filters.Remove("customSkuSearch");
    // apply skuFilters to the query yourself
}

return await base.GetPaginatedProductList(filterDTO, query, authorize);
```
