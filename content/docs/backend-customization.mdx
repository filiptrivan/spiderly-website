...
For custom authorization checks — returns `401 Unauthorized` with your message. See the [Authorization Service](#authorization-service) section for a usage example.

### Unhandled Exceptions

Any exception that isn't a `BusinessException` or `UnauthorizedException` returns a generic error message to the client (never exposing internal details) and triggers email/Telegram notifications in production via `IExceptionNotificationDispatcher`.
