...
- `OnAfter{Entity}ListDelete()` - Execute logic after deletion (cascades included), still inside the delete transaction. Queries observe the post-delete state, so this is the place to recompute denormalized aggregates; writes commit or roll back atomically with the delete.
- `OnAfter{Entity}Delete()` - Per-id variant; defaults to forwarding to `OnAfter{Entity}ListDelete` with a one-element list.
...
The `Mapper` class is generated as a partial class. You can add your own custom mapping methods here.

**Adding mappings — `Customize*` partial hooks.** Every generated config method declares a matching `static partial void Customize{MethodName}(TypeAdapterConfig config)` hook and calls it before returning. Implement it to add custom mappings on top of the generated ones, in real compiler-checked C# — including the null guards that projections through optional navigations require:
...
public static partial class Mapper
{
    ...
    {
        // ProductVariant is an optional navigation — the null guard is mandatory:
        // unguarded access LEFT JOINs to NULL and crashes EF's materializer.
...
Use `config.ForType<...>()` (get-or-extend) in hooks, never `NewConfig` (replace) — the generated mappings are already on the config when the hook runs. An unimplemented hook compiles away to nothing.

Precedence: `.Map` is *first-registration-wins* — a hook `.Map` targeting a member the generator already mapped is a silent no-op, while `.Ignore` does suppress it. Hooks add and suppress; to *replace* how a generated member is mapped, use the same-name full-method override below.

Generated configs also disable Mapster's convention *flattening*: a DTO property named like a navigation chain (e.g. `ShippingTierIsBulky`) is **not** silently resolved through `ShippingTier.IsBulky` — an unmapped property stays at its default until you wire it deliberately through a hook.

**Replacing mappings — same-name override.** If you define a method with the same name as a generated one (e.g., `{Entity}DTOToEntityConfig`, `{Entity}ToDTOConfig`, `{Entity}ProjectToConfig`), the generator will skip that method and use yours instead.
...
Marks a property as **server-owned**: clients can read it but never write it (the same idea as OpenAPI's `readOnly`). The property stays in the read `{Entity}DTO` — so it's still returned by GET endpoints and shown in the list table — but Spiderly closes its write path: no inbound validation rule is generated, the generated DTO→entity Mapster config `.Ignore()`s it (so a crafted payload can't set it), and it gets no editable control in the details form.

Use it for values only your backend assigns — usage counters, denormalized aggregates, computed timestamps. Pair it with `[Required]` to document the non-null column without generating an unsatisfiable rule: `[Required]` alone would emit a `NotEmpty` check the form can never satisfy (the form never sends the field), so every save would fail with a 422.
...
public class DiscountCode : BusinessObject<int>
{
    ...
    [ReadOnly]
    [Required]
    public int TimesUsed { get; set; } // only backend code increments it; clients read only
}
```

Three attributes cover three different intents, so reach for the one that matches yours:

| Attribute           | Read DTO | Writable | In form                                       |
| ------------------- | -------- | -------- | --------------------------------------------- |
| `[ReadOnly]`        | yes      | no       | no (read-only value shown in the list)        |
| `[UIDoNotGenerate]` | yes      | yes      | hidden (a custom UI or another flow fills it) |
| `[ExcludeFromDTO]`  | no       | no       | no (invisible to the whole API surface)       |
