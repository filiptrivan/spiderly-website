---
title: Spiderly Configuration
description: Configure source generator behavior and API settings with .spiderly/config.json.
---
...
```json title="YourApp.Business/.spiderly/config.json"
{
  "generators": {},
  "api": {
    "routePrefix": "/api"
...
<Callout>JSON parsing is case-insensitive — `"generators"` and `"Generators"` both work.</Callout>

## Full Example

```json title=".spiderly/config.json"
{
  "generators": {
...
<Callout>
  This switch controls generated **artifacts** only. Entity-shape validation — the
  [build diagnostics](/docs/build-diagnostics) that tell you an entity is malformed — runs in a
  separate generator that produces no output and **cannot be disabled**, so it is not in the tables
  below. Declining code you don't want should never also decline the checks that say your entities are
  well-formed.
</Callout>
