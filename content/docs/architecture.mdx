...
|---|---|---|
| ServicesGenerator | `{Entity}EntityService.generated.cs` | Per-entity CRUD operations, data retrieval, Excel export |
...
- Business logic overrides in entity service classes (e.g., `ProductEntityService`)
...
ServiceBase                      ← Framework (Spiderly NuGet package)
        ↓
{Entity}EntityServiceGenerated   ← Generated per entity (readonly, rebuilt every build)
        ↓
{Entity}EntityService            ← Your code (override virtual methods here)
```
