...
- `CreatedAt` / `ModifiedAt` — Stamped by the framework inside `SaveChanges`. Both are set on insert and `ModifiedAt` is refreshed on every update; `CreatedAt` is immutable once the row exists. On insert, an explicitly-assigned `CreatedAt` is preserved (the framework stamps it only when left unset), so a data import or migration can keep the source system's original creation dates.
