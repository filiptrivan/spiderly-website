...
`traceId` is the request's W3C trace id, present only on reportable errors — the rows logged at `Error` in the table below, the same classification `SpiderlyExceptionClassifier.IsExpected` exposes to error-tracker filters — so ordinary validation feedback never looks like an incident. Surface it to users as an error reference: it finds the matching server logs and error-tracker events (e.g. Sentry's `trace:` search). The Angular admin's HTTP interceptor appends it to any error toast whose response carries one, and every response includes the same id in the `X-Request-Id` header via `RequestIdMiddleware`; upstream callers correlate by sending a standard `traceparent` header (a client-sent `X-Request-Id` is ignored).
