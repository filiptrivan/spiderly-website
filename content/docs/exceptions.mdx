...
```ts
{
  ...
  traceId?: string;            // W3C trace id — support reference, reportable errors only
  exception?: string;          // full stack trace, development only
}
```

`traceId` is the request's W3C trace id, populated only for reportable errors — unhandled 500s and `SecurityViolationException`, the same classification `SpiderlyExceptionClassifier.IsExpected` exposes to error-tracker filters. Surface it to users as an error reference: it correlates the response with your server logs and error-tracker events (e.g. Sentry's `trace:` search), while expected 4xx responses never carry it, so ordinary validation feedback doesn't look like an incident. The Angular admin's HTTP interceptor appends it to the 403 and unexpected-error toasts automatically, and every response (not just errors) carries the same id in the `X-Request-Id` header via `RequestIdMiddleware` — one correlation id everywhere. Upstream callers who need to correlate their own logs supply a standard `traceparent` header; a client-sent `X-Request-Id` is deliberately ignored.
