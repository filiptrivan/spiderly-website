...
- **`?.` is not a substitute for the guard** when you project a Spiderly control. It hands the control `undefined`, which every control's template is written to survive, but your own components have to be equally tolerant if you rely on it.
