...
<Callout type="warn">
  Cookie-based authentication requires the [`AuthGuard`](/docs/attributes/general#authguard) attribute's CSRF protection. All state-changing requests (`POST`, `PUT`, `DELETE`, `PATCH`) must include the `X-CSRF: "1"` header.
</Callout>
