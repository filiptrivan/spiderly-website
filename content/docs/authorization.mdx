...
<Callout type="warn">
  Cookie-based authentication requires the [`[AuthGuard]`](#authguard) attribute's CSRF protection.
  All state-changing requests (`POST`, `PUT`, `DELETE`, `PATCH`) must include the `X-CSRF: "1"`
  header.
</Callout>
...
<Callout type="warn">
  ...
  `[HasPermission]` handler, the app **fails at startup**: _"Spiderly authentication is enabled, so
  `[HasPermission]` materializes a `PermissionRequirement` policy, but no `IAuthorizationHandler` that
