...
| `defaultSortField`                    | `string`                                  | —                   | Sort applied while the user has no sort of their own — shows as a normal header arrow; persisted user sort wins; un-sorting (tri-state header click, Clear filters) returns to it |
...
**Views** (`[views]`) are saved filter presets rendered as tabs: each view's `apply` receives the cleared store and writes with `setAndCommit`; a view that depends on _now_ (e.g. "received today") declares `transient: true` so selecting it re-derives instead of restoring a stale answer. Column layout persists per view.
