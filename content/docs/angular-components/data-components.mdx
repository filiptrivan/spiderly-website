...
The range anchor is the last checkbox you clicked, and it resets whenever the visible rows change (page, sort, or filter change), so a range never spans pages. Rows already in the target state are skipped, and every actual change emits `onRowSelect`/`onRowUnselect` exactly as a single click would.
