...
Whatever you return is slugified defensively (lowercased, diacritics transliterated — `đ→dj`, `š→s`, `ü→u` — separators collapsed, capped at 60 chars), so a raw display name is as safe to return as a clean slug. The resulting key:
