...
Whatever you return is slugified defensively — lowercased, diacritics transliterated (`š→s`, `ü→u`, plus the stroked letters and ligatures Unicode decomposition cannot fold: `đ→dj`, `ł→l`, `ø→o`, `æ→ae`, `ß→ss`), separator runs collapsed, and capped at 60 characters on a word boundary rather than mid-word. So a raw display name is as safe to return as a clean slug. The resulting key:
