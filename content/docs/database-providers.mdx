...
Connection strings are stored in [.NET user secrets](https://learn.microsoft.com/en-us/aspnet/core/security/app-secrets) and configured automatically during `spiderly init`.
...
`spiderly init` detects whether the database service is already running. If not, it offers to install it automatically.

| OS | PostgreSQL | SQL Server |
| --- | --- | --- |
| Windows | Chocolatey (`choco install postgresql`) | Chocolatey (`choco install sql-server-express`) |
| macOS | Homebrew (`brew install postgresql`) | Docker container |
| Linux | Docker container | Docker container |

If the package manager (Chocolatey, Homebrew) isn't installed, the CLI will offer to install it first.
