...
Under `<Nullable>enable</Nullable>` an entity property's annotation *is* its column's nullability — EF reads a navigation's nullability as the relationship's requiredness, and nothing configures scalar requiredness separately. So an annotation that disagrees with `[Required]` rewrites your schema without anyone asking: the next migration alters the column, and a save that legitimately omits the value writes a default instead of `NULL`.

```csharp
// Reported: no [Required], so the relationship is optional — but the annotation
// makes EF create a NOT NULL foreign key. A task saved without a category then
// writes 0 and dies on a foreign-key violation.
