...
`[Required]` is what Spiderly turns into a column's nullability. When your nullable annotation contradicts it, the C# type lies about the schema — and which side wins depends on the property kind, so neither reading is safe to rely on.

For **scalars** the annotation wins outright: nothing configures scalar requiredness, so EF's convention reads the annotation and your column silently changes. For **navigations** Spiderly configures `.IsRequired([Required] != null)` explicitly, so the attribute wins and it is the annotation that misinforms whoever reads the entity. Either way the two must tell the same story.
